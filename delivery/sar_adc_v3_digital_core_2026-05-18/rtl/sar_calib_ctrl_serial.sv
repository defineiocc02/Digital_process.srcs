...
//   REF_WEIGHT_LSB: Fixed-point value of Bit 0 reference weight (default 256)
//
// Ports         :
//   clk           : System clock
...
module sar_calib_ctrl_serial #(
    ...
    localparam int PROTECT_START_BIT     = CAP_NUM - 2;
    localparam int PROTECT_LOW_BIT       = CAP_NUM - 3;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for(int i=0; i<CAP_NUM; i++) begin
                if (i <= MAX_CALIB_BIT)
                    shadow_weights[i] <= REF_WEIGHT_LSB <<< i;
                else
                    shadow_weights[i] <= '0;
            end
