...
// Note          : 1. [Critical Fix] Bit 0-5 weights are initialized by DUT reset.
//                    The calibration algorithm skips lower 6 bits, so reconstruction RAM
//                    must own these baseline values to avoid systematic offset.
...
module tb_sar_adc_top;
    ...
    initial begin
        ...
        $display("|  [INFO]   | LSB Init        | DUT reset loaded Bit 0-5 ideal weights.");
        ...
        for (k=0; k<CAP_NUM; k++) begin
            ...
            // Bit 0-5 are reset-loaded by DUT, should also pass (as long as K is close to 1.0)
